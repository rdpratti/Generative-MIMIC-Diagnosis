...
class MedicalRAGRetriever:
    ...
    def build_index(self, train_df, text_column='text', 
                    save_path=None, use_cache=True, 
                    force_rebuild=False):
        """
        ...
        """
        #cache_path = Path('E:/vscode-projects/thesis/src/rag_index')
        cache_path = Path('~/projects/Thesis/src/rag_index')
    
        # Check if cache exists
        cache_files = [
            cache_path / "faiss_index.bin",
            cache_path / "examples.parquet",
            cache_path / "embeddings.npy"
        ]
        cache_exists = all(f.exists() for f in cache_files)
    
        # Decide whether to load or build
        if use_cache and cache_exists and not force_rebuild:
            # Load from cache
            if self.logger:
                self.logger.info("=" * 60)
                self.logger.info("LOADING RAG INDEX FROM CACHE")
                self.logger.info(f"Cache location: {cache_path}")
                self.logger.info("=" * 60)
        
            try:
                self.load_index(cache_path)  # ← Use your existing method!
            
                if self.logger:
                    self.logger.info(f"✅ Successfully loaded cached index")
                    self.logger.info(f"   Examples: {len(self.examples_df)}")
                    self.logger.info(f"   Embeddings shape: {self.embeddings.shape}")
                    self.logger.info(f"   FAISS vectors: {self.index.ntotal}")
                    self.logger.info(f"   Examples per class:")
                    for code, count in self.examples_df['icd10_code'].value_counts().items():
                ...
                if self.logger:
                    ...
                    self.logger.info("Falling back to building from scratch...")
        ...
        self.logger.info(f"Embedding model device: {device}")
        self.logger.info(f"Embedding model name: {self.embedding_model.__class__.__name__}")
